...
async function fetchPlaces(token) {
    try {
        ...
        if (list) list.innerHTML = '<p class="empty-state">Unable to load places.</p>';
    }
}
...
async function fetchPlaceDetails(token, placeId) {
    try {
        ...
        if (d) d.innerHTML = '<p class="empty-state">Place not found.</p>';
    }
}
...
function displayPlaceDetails(place) {
    ...
    if (place.amenities && place.amenities.length > 0) {
        const tags = place.amenities.map(function(a) {
            return '<span class="amenity-tag">' + (a.name || a) + '</span>';
        }).join('');
        amenitiesHTML = '<div class="amenities-list">' + tags + '</div>';
    }
    const owner = place.owner ? place.owner.first_name + ' ' + place.owner.last_name : 'N/A';
    section.innerHTML =
        '<div class="place-details"><div class="place-info">' +
        '<h2>' + (place.title || place.name || 'Unnamed') + '</h2>' +
        ...
        '<p><strong>Amenities:</strong></p>' + amenitiesHTML +
        '</div></div>' +
        '<div class="reviews-section"><h3>Reviews</h3>' +
        '<div id="reviews-list"><p class="loading">Loading reviews...</p></div></div>';
}
...
async function fetchReviews(token, placeId) {
    try {
        ...
        if (!reviews || reviews.length === 0) { reviewsList.innerHTML = '<p class="empty-state">No reviews yet.</p>'; return; }
        reviewsList.innerHTML = '';
        reviews.forEach(function(review) {
            const card = document.createElement('div');
            card.classList.add('review-card');
            const user = review.user ? review.user.first_name + ' ' + review.user.last_name : 'Anonymous';
            card.innerHTML =
                '<p class="reviewer">' + user + '</p>' +
        ...
        if (r) r.innerHTML = '<p class="empty-state">Could not load reviews.</p>';
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (page === 'index.html' || page === '') {
        ...
        if (pf) pf.addEventListener('change', function(e) { filterPlacesByPrice(e.target.value); });
    }
    ...
    if (page === 'place.html') {
        ...
        const ar = document.getElementById('add-review');
        if (ar) ar.style.display = token ? 'block' : 'none';
        ...
        if (rf && token) {
            rf.addEventListener('submit', async function(e) {
                ...
                handleResponse(await submitReview(token, placeId, text, rating), rf);
            });
        }
    }
